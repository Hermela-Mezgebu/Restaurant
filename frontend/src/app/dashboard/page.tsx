...
export default function DashboardPage() {
  ...
  const user = getUser() as StaffUser | null;
  ...
  const getParams = () => {
    ...
    if (dateFilter === 'week') {
      ...
      return {
        date_from: todayStr,
        date_to: `${weekLater.getFullYear()}-${wlMm}-${wlDd}`,
      };
    }
    ...
    return {
      date_from: todayStr,
      date_to: `${monthLater.getFullYear()}-${mlMm}-${mlDd}`,
    };
  };
  ...
  useEffect(() => {
    const fetchReservations = async () => {
      ...
      try {
        ...
        setError(
          'Failed to load restaurant reservations.'
        );
      } finally {
        setLoading(false);
      }
